...
namespace ArqComputadorasProyecto
{
    internal class Program
    {
        static void Main(string[] args)
        {
           ...
           var numeroPrimoParallelForeach = ListaNumerosParallel(numeros);
           watchForParallel.Stop();
            ...
            Console.WriteLine($"foreach Parallel | Total de numeros primos :{numeroPrimoParallelForeach.Count} | Tiempo Total : {watchForParallel.ElapsedMilliseconds} ms");
            Console.ReadLine();
        }

        private static IList<int> ListaNumerosPrimos(IList<int> numeros) => numeros.Where(EsPrimo).ToList();

        private static IList<int> ListaNumerosParallel(IList<int> numeros)
        {
            var numerosPrimos = new ConcurrentBag<int>();
